...
const roleSchema = Joi.object({
  ...
  description: Joi.string().min(5).max(100).error(createError.BadRequest("توضیحات نقش صحیح نمیباشد")),
...
const permissionSchema = Joi.object({
  ...
  description: Joi.string().min(5).max(100).error(createError.BadRequest("توضیحات سطح دسترسی صحیح نمیباشد")),
});
